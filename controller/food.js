...
exports.updatefood = async (req, res) => {
  try {
    const Foodupdate = await FoodModel.findOneAndUpdate(
...
exports.starRating = async (req, res) => {
  ...
  var totalrating = food_.rating?.reduce(
    (sum, curr) => sum + Number(curr.rating ? curr.rating : 0),
    0
  );
  ...
  if (!filter) {
    food_.totalrating =
      ...
      (food_.rating.length >= 1 ? food_.rating.length + 1 : 1);
    const Foodupdate = await FoodModel.findOneAndUpdate(
      { _id: req.params.foodid },
      {
        $push: {
    ...
    for (var i in food_.rating) {
      if (food_.rating[i].user === user) {
        ...
        food_.totalrating =
          (totalrating - abc + req.body.rating) / food_.rating.length;
      }
    }
  }
  food_.save();
